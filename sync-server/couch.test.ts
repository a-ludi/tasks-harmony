...
describe('couchGet', () => {
  it('returns null for 404', async () => {
    mockFetch(mock(() => Promise.resolve(new Response(null, { status: 404 }))));
  ...
  it('returns parsed doc for 200', async () => {
    ...
    mockFetch(mock(() => Promise.resolve(new Response(JSON.stringify(doc), { status: 200 }))));
    const result = await couchGet('push-subscriptions', 'sub-abc');
  ...
  it('throws on 400', async () => {
    mockFetch(mock(() => Promise.resolve(new Response(null, { status: 400 }))));
...
describe('couchFind', () => {
  ...
  it('throws on 400', async () => {
    mockFetch(mock(() => Promise.resolve(new Response(JSON.stringify({ error: 'bad_request' }), { status: 400 }))));
