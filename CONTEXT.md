...
**Sync**:
The process of keeping the full app state consistent across devices via the app server. Sync is end-to-end encrypted — the server stores opaque ciphertext and never sees plaintext app data. Sync runs automatically: a pull happens at startup, and a debounced push (10 s delay) fires after every write. A manual "Sync now" button on the Profile page triggers a push immediately. Authentication uses a post-quantum challenge-response protocol (ML-DSA-87 signatures). Encryption uses a KEM-based hybrid scheme (ML-KEM-1024 for key encapsulation, AES-256-GCM for the blob). The user's sync identity and key are stored in a dedicated IndexedDB `credentials` store, separate from app state, and are never included in app state exports. App state exports default to encrypted format using the same key; the chosen format is persisted in `localStorage` so both the Profile page and the backup reminder banner stay in sync.
_Avoid_: Backup, upload, save, WebDAV

**Backup Reminder**:
A recurring prompt to export the app state, shown as a dismissible banner at the top of the Dashboard. Triggered when a scheduled backup is due — the schedule is configurable in the Profile page as never / daily / weekly / monthly (default daily). The due date is computed by rounding the last action (export or dismiss) down to local midnight, then adding the period; the reminder fires the first time the app is opened on or after that date. On first app open, the dismissed-at timestamp is seeded to now so the reminder waits one full period before appearing. Actions: "Export now" (downloads the backup and resets the clock), "Remind me later" (hides for the session only), "×" (snoozes for one full period). The export format (encrypted or plain) is shared with the Profile page export and persisted in `localStorage`.
_Avoid_: Alert, notification, push reminder
