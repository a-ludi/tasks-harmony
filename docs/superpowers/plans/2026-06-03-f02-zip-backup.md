...
### Task 2: Add pristine-state helper

**Files:**
- Modify: `src/backup/backup.ts`
- Modify: `src/backup/backup.test.ts`

- [ ] **Step 1: Add failing tests for isAppStatePristine**

Append to `src/backup/backup.test.ts`:

```ts
import { isAppStatePristine } from './backup';
import type { Pack, Completion } from '@/types';
...
Expected: FAIL — `isAppStatePristine` not exported.

- [ ] **Step 3: Add isAppStatePristine to backup.ts**

Append to `src/backup/backup.ts`:
...
```bash
git add src/backup/backup.ts src/backup/backup.test.ts
git commit -m "feat: add isAppStatePristine helper for conditional import confirmation"
```

---

### Task 3: Wire export into ProfilePage
...
### Task 4: Wire import into ProfilePage
...
Inside the Backup section added in Task 3, after the Export button, add:
